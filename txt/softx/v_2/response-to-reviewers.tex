...
Two numerical results from the submitted version have changed:

\begin{enumerate}[leftmargin=*]
\item \textbf{Speedup.} The parallel-efficiency table requested by Reviewer~1 shows $43\%$
efficiency at $N=44$, so the ideal $8\times$ speedup is now claimed from $N=52$: the two samplers
...
scale we can safely assume as within reach of current leadership-class systems. The revised paragraph
...
points. These results constitute the new Fig.~2 and the paragraph \emph{A rugged family where
...
\emph{Architecture}, \emph{Performance} and the Fig.~1 caption now contain exactly this claim.
Below the crossover
...
controller receiving one object instead of $P$. We have implemented it, and it overtakes the direct
