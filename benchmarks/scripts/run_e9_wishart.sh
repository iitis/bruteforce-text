...
# optimum. Small alpha is the hard regime; alpha = 0.5 is the easy control.
#
# Roughly 6 minutes at N=40 for 15 instances: seconds of GPU per certificate, ~15 s of CPU per
# dSB run. The analytic E_0 also allows a GPU-free re-check via --skip-bruteforce.
#
# Usage:
#   ./scripts/run_e9_wishart.sh
#   ./scripts/run_e9_wishart.sh --size 40 --replicas 5 --alphas "0.2 0.3 0.5"
set -euo pipefail
