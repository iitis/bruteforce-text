...
Two numbers from the submitted version have changed, and we state them up front:
...
The projection now rests on measurement rather than on argument. Table~5 reports the
...
method-specific budgets; we do not equate a dSB integration step with an SA sweep.
...
The revision also adds the complementary case, a family on which the heuristic \emph{fails}.
...
bound. This is the plugin's intended role made concrete (new paragraph \emph{A rugged family
where certification is decisive}; driver \texttt{benchmarks/exp\_wishart.py}, launcher
\texttt{run\_e9\_wishart.sh}, re-checkable without a GPU on the planted arm).
...
CPU-enumerable sizes and independent \texttt{float64} recomputation at benchmark scale, as
correctness checks rather than claims about comparative performance.
...
The table dates the full $\approx\!8\times$ speedup from $N=52$ rather than from
...
rather than superlinear scaling. The revision also documents why the $N=40$ row overstates the
...
\textbf{The data now ships with the article rather than in a separate repository.} The
...
hierarchical merge. This is a scheduling floor rather than the total cost of a payload-bearing
...
Since the measurement follows Ray's ordinary task-return path rather than forcing a particular
storage mechanism, it does not separately identify serialization, transport, scheduling or
...
scheduling; both strategies ship in the plugin and the released sampler selects between them on
that measured threshold.

The revision also defines what the reported times measure. For the single-GPU sampler the timer
...
throughput, so on this hardware single precision buys working-set size rather than speed, and
...
The original publication not only lists the plugin and notes that it can be GPU-accelerated; it
also reports a benchmark of the GPU sampler on $1$, $2$, $4$ and $8$ NVIDIA A100 GPUs, and
describes the plugin as usable up to about $50$ variables. Neither ``first-class Omnisolver
...
This is consistent with the absence of inter-worker communication during the search, and
localizes the variability to the two-node/second-host path rather than to a particular
...
intended scope} closes on that, and extending the study past two node boundaries is now one of
