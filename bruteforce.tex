...
beyond that point the distributed sampler amortizes the work by powers of two
in the number of workers (and on our hardware crosses the break-even region
around $N\!\approx\!46$, see Fig.~\ref{fig:runtimes}). Performance-critical
...
\caption{Wall-clock runtime of \texttt{omnisolver-bruteforce} in distributed
    mode on all-to-all random Ising instances with $J_{ij},h_i\sim
    \mathcal{U}[-1,1]$, measured on a local in-house cluster of $8\times$
    NVIDIA H100 (96\,GB) GPUs (two nodes $\times$ 4 GPUs). Each data point
    is a single solver run on one fixed instance per system size $N$. The
...
we cross-check an SBM run on the same instances: a chaotic-variant
simulated bifurcation solver running on a single H100 GPU with
$2^{12}=4096$ parallel replicas and $3{,}000$ integration steps, reporting
the lowest-energy replica. The recomputed energies of the brute-force and
...
under the empirical $t_{N+1}=2\,t_N$ rule and should be expected to drift
by a small constant factor on different stacks. Reported runtimes
...
The authors declare the following relationships that could be perceived as
competing interests: B.~Gardas, J.~Paw{\l}owski, and {\L}.~Pawela are
affiliated with Quantumz.io Sp.~z~o.\,o. (see addresses). Quantumz.io
develops commercial QUBO solvers, including the Simulated Bifurcation
Machine used in this paper for cross-checking ground-state energies. The
authors received no payment or in-kind support from any party for the
preparation of this manuscript beyond the funding declared in
Acknowledgements, and the design, results, and conclusions of this work
